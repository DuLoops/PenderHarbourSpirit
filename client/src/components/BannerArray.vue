        ...
        <div id="content">
            <div id="title">
                ...
                <div id="rootDirectory_and_location_mobile" v-if="isMobileView">
                    <p v-if="isRootDirectoryAvailable">{{rootDirectory}}</p> 
                    <p v-else>Input Root Directory</p> 
                </div>
    ...
    export default {
        ...
        mounted() {
            ...
            if(this.img === undefined || this.img === "") {
                ...
                //Generate Random Number (Values: 0-11)
                var randomIndex = Math.floor(Math.random() * 12);
                
                // this.img will be an array
                var randomLink = this.img[randomIndex];

                banner.style.backgroundImage = `url(${randomLink})`;
